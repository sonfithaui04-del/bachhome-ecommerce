# 📚 Hướng Dẫn Sử Dụng & Giải Thích Chi Tiết Hệ Thống BachHome
...
- **Core Backend**:
  ...
  - **Order & Product Services**: Xử lý logic đặt hàng, quản lý sản phẩm, tồn kho.
...
```
Frontend (React) ◄──HTTP──► API Gateway (8080) ◄──HTTP──► Services (Auth, Product, Order...)
                                                                  ▲
                                                                  │ (Async Messaging)
                                                                  ▼
                                                               RabbitMQ
